...
cachedRequest.setCacheDirectory(path.join(__dirname, '..', '..', '.request-cache'));
    ...
    return new Promise(function (resolve, reject) {
        var assetRefs = _(data.assetData).filter(function (v){
            ...
            reqUrls = _(assetRefs).map(function (v) {
                var rtn = {};
                rtn[v.id] = config.services.host + '/MediaInquiryService/1.0/rest/media/' + v.attributes.externalid;
                return rtn;
            }),
            requests,
            a = {};
